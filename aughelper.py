...
def padimage(dirName, extension, image, top, bottom, left, right, padcolor=[0, 0, 0]):
    ...
    cv2.imwrite(dirName+"/Padded-"+ "("+str(top)+","+str(bottom)+")"+"*"+"("+str(left)+","+str(right)+")"+
...
def cropimage(dirName, extension, image, y1, y2, x1, x2):
    ...
    cv2.imwrite(dirName+"/Cropped-"+"["+str(y1)+", "+str(y2)+"]"+" * "+"["+str(x1)+", "+str(x2)+"]"+extension,
                croppedimage)
...
def multiplywith(dirName, extension, image, B, G, R):
    ...
    cv2.imwrite(dirName+"/Multiplied with-"+"("+str(B)+", "+str(G)+", "+str(R)+")"+extension, mulimage)
